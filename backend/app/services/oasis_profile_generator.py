...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        zep_api_key: Optional[str] = None,  # kept for interface compat, ignored
        ...
        # Graphiti client for enriching entity context
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Graphiti搜索第 {attempt + 1} 次失败: {str(e)[:80]}, 重试中...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Graphiti搜索在 {max_retries} 次尝试后仍失败: {e}")
            return None

        try:
            edge_result = search_graphiti()

            # 处理搜索结果 (Graphiti returns List[EntityEdge])
            all_facts = set()
            if edge_result:
            ...
            logger.info(f"Graphiti检索完成: {entity_name}, 获取 {len(results['facts'])} 条事实")
